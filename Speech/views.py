...
class Predict(APIView):
    ...
    @staticmethod
    def predict(log_mel_spectrogram):
        pred = SpeechConfig.model.predict(log_mel_spectrogram)
        return pred

    def post(self, request):
        audio = request.data['audio']
        log_mel_spectrogram = self.extract_log_melspectrogram(waveform=audio)
        pred = self.predict(log_mel_spectrogram)
        decoded_pred = self.decode_batch_predictions(pred)

        return Response(decoded_pred)
